...
module Conversion (BaseC: Base) =
  ...
  let rec decompose n =
      if n=0 then []
      else decompose (n/BaseC.base)@[(n mod BaseC.base)]

let rec puissance_b n b=
      if n=0 then 1 else b*puissance_b (n-1) b
  ...
  let  recompose l =
    ...
    let rec aux i l =
      match l with 
        |[t]->t
        |t::q -> t*(puissance_b i BaseC.base ) + aux (i-1) q
    in aux ((List.length l)-1) l
end
