...
struct
  ...
  let rec conforme arb =
    match arb with 
    |Vide -> true 
    |Noeud(_,a)-> (List.length a =BaseA.base) && (fils_conforme a)

  and fils_conforme l =
    match l with 
    []-> true 
    |t::q -> (conforme t )&& (fils_conforme q)
  ...
  (* L'ensemble de départ et l'ensemble d'arrivée sont conformes à la base *)
    let rec appartient l arb =
      match arb with 
        ...
        |Noeud(b,l_fils)-> 
                        (match l with
                            |[] -> b (*SI b=vrai, le nombre est dans l'arbre*)
                           |t::q -> appartient q (get t l_fils)
                        )

  (* ajouter : int list -> arbre -> arbre *)
  (* Ajoute un entier, décomposé en base B, à un ensemble d'entier codé avec un arbre *)
  (* L'ensemble de départ et l'ensemble d'arrivée sont conformes à la base *)
  let rec ajouter n arb =
    match n with 
    |[] ->(match arb with 
            |Vide ->Noeud(true, l_taille BaseA.base Vide)
            |Noeud(false, lf) -> Noeud(true, lf)
            |Noeud(true, lf) -> Noeud(true, lf))
    |t::q ->(match arb with 
                Vide -> Noeud(false, set t (l_taille BaseA.base Vide) (ajouter q Vide)) (*Si c'est vide, on crée des fils vide avec l_liste et on initialise l'element t avec la fonction ajoute et l'arbre vide*)
                |Noeud(b,lf) -> Noeud(b,(set t lf (ajouter q (get t lf)))) (*Si on est sur un NOeud, on modifie l'elemnt t de la liste des fils avec la fonctin ajouter, prenant en argumant l'arbre qui était présent avant le passage de notre fonction.*)
                )
  ...
  let rec retirer n arb =
    match arb with 
      |Vide -> Vide
      |Noeud(b,lf) -> match n with 
                        |[] -> if lf=(l_taille BaseA.base Vide) then Vide else Noeud(false, lf)
                        |t::q -> (let l =set t lf (retirer q (get t lf)) in if (l = (l_taille BaseA.base Vide) && (not b)) then Vide else Noeud(b,l))
