...
let%test_unit "testP3E" = 
 ...
 let%test_unit "testP4E" = 
 try
  let _ = compiler (pathFichiersRat^"testP4E.rat") 
  in raise ErreurNonDetectee
  with
  | TypeInattendu _ -> ()

  (*UNe boucle for banale.*)
  let%test_unit "testFor1" = 
   let _ = compiler (pathFichiersRat^"testFor1.rat") in ()

    (*Cas ou la variable définie n'est pas un etnier mais un rationaelle*)
  ...
  try
   ...
   (*Cas ou la comparaison sur la deuxième expression n'est pas du bone type, dans ce cas, comparaison entier/boolean*)
   ...
   try
    ...
    (*Cas ou la dernière expression n'est pas compatible avec le type int, ici int/rat*)
    let%test_unit "testFor4" = 
   try
    let _ = compiler (pathFichiersRat^"testFor4.rat") 
    in raise ErreurNonDetectee
    with
    | TypeBinaireInattendu _ -> ()
