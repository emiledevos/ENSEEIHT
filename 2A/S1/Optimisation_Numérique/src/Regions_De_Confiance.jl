...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    while !(c0 | c1 | c2 | c3)
        ...
        maj=false
        if algo_pas =="cauchy"
            s=cauchy(gradf(xk),hessf(xk),d)
        else
            s=gct(gradf(xk),hessf(xk),d,max_iter=max_iter_gct)
        ...
        mk(s)=f(xk)+ g'*s+ (1/2)*s'*h*s

        
        #calcul de l'approximation de la fonction f dans la région de confiance
        
        rk=(f(xk)-f(xk+s))/(mk(zero(g))-mk(s))
        ...
        if maj
            ...
            c1 = norm(s) < epsilon * max(tol_rel*norm(xk),tol_abs)
            c2 = norm(f(xk+s)-f(xk)) < epsilon * max(tol_rel*norm(f(xk)),tol_abs)
        end  
