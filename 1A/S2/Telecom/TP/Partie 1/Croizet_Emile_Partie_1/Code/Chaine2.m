...
%% Filtrage par le filtre dex_recu = filter(Filtre_reception,1,x_canal, [], 2); réception

x_recu = filter(Filtre_reception,1,x_filtre_bruit, [], 2);

%% Détection

N0 = 6; % selon le diagramme de l'oeil
...
pathname = "Chaine2_figure";

% Diagramme de l'oeil
nom = "Chaine_2_Diagramme_de_oeil_taille_" + num2str(Taille);
...
x_TEB = kron(ones(length(consigne_variable),1), x_filtre) + bruit_variable; % Faire les 9 colonnes

%%% Signal en sortie du filtre de réception
x_TEB = filter(Filtre_reception,1,x_TEB,[],2);
...
%%% TEB théorique
TES =   2*((M-1)/M) * qfunc( sqrt( (3*log2(M)/(M^2 - 1)) * 10.^(consigne_variable/10)));
TEB_theorique = TES/log2(M);
%% Tracé
% Affichage du TEB qui varie

nom = "Chaine_2_variation_TEB_taille_" + num2str(Taille);
...
nom = "Chaine_2_variation_Diagramme_oeil_taille_" + num2str(Taille);
