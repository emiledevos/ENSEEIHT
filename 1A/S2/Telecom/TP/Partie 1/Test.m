...
%% Simulation d'un canal (à l'aide d'un filtrage passe bas)

% On rajoute des 0 à la fin pour la gestion du retard
x_filtre_bruit = [x_filtre_bruit zeros(2,floor((Ordre-1)/2))];

% On filtre pour le canal
x_canal = filter(h_pb,1,x_filtre_bruit, [], 2);
...
x_recu = filter(Filtre_reception,1,x_canal, [], 2);
...
x_TEB = [x_TEB zeros(length(consigne_variable),floor((Ordre-1)/2))];

%% Traitement mais pour l'erreur qui varie

%%% On filtre pour le canal
x_TEB = filter(h_pb,1,x_TEB,[],2);

%%% On enlève le début qui correspond à la phase transitoire du filtre
x_TEB = x_TEB(:,floor((Ordre-1)/2)+1:end);
...
semilogy(0:length(TEB_variable)-1,TEB_variable,"--squareb");
hold on;
semilogy(0:length(TEB_variable)-1,TEB_theorique,":or");
xlabel("E_{b}/N_{0}");
ylabel("TEB");
